@extends('layouts.blackbox.main')
@section('content')

  <div class="container">

    <div class="container-header">
      <div class="row">
        <div class="col-lg-6 col-sm-12">
          <div class="title">
            งาน
          </div>
        </div>
      </div>
    </div>

    <div class="shop-nav-group space-top-bottom-20 clearfix">

      <div class="shop-nav small">
        <div class="shop-nav-image">
          <a href="{{$shopUrl}}job_add">
            <img src="/images/common/job.png">
          </a>
        </div>
        <div class="shop-nav-info">
          <a href="{{$shopUrl}}job_add">
            <h4 class="shop-nav-title">แสดงรายชื่อผู้ที่สนใจงานของคุณ</h4>
          </a>
        </div>
      </div>

      <div class="shop-nav small">
        <div class="shop-nav-image">
          <a href="{{$shopUrl}}job_add">
            <img src="/images/common/job.png">
          </a>
        </div>
        <div class="shop-nav-info">
          <a href="{{$shopUrl}}job_add">
            <h4 class="shop-nav-title">ลงประกาศงาน</h4>
          </a>
        </div>
      </div>

      <div class="shop-nav small">
        <div class="shop-nav-image">
            <a href="{{$shopUrl}}branch_add">
              <img src="/images/common/plus.png">
            </a>
        </div>
        <div class="shop-nav-info">
          <a href="{{$shopUrl}}branch_add">
            <h4 class="shop-nav-title">เพิ่มสาขา</h4>
          </a>
        </div>
      </div>

      <div class="shop-nav small">
        <div class="shop-nav-image">
            <a href="{{$shopUrl}}department_add">
              <img src="/images/common/plus.png">
            </a>
        </div>
        <div class="shop-nav-info">
          <a href="{{$shopUrl}}department_add">
            <h4 class="shop-nav-title">เพิ่มแผนก</h4>
          </a>
        </div>
      </div>

    </div>

    <div class="line"></div>

    <div class="list">

      @if(!empty($pagination['data']))

        <div class="row">

          @foreach($pagination['data'] as $data)

          <div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
            <div class="card">
              <div class="image">
                <a href="{{$detailUrl}}{{$data['id']}}">
                  <div class="product-image" style="background-image:url({{$data['_imageUrl']}});"></div>
                </a>
              </div>
              <div class="product-detail">
                <a href="{{$detailUrl}}{{$data['id']}}">
                  <div class="product-title">{{$data['_name_short']}}</div>
                </a>
                <div class="price">
                  {{$data['_price']}}
                </div>
              </div>
              <div>
                <a href="{{$detailUrl}}{{$data['id']}}"><div class="button wide-button">แสดงสินค้านี้</div></a>
              </div>
            </div>
          </div>

          @endforeach

        </div>

        @include('components.pagination') 

      @else

      <div class="shop-notice text-center">
        <img src="/images/common/job.png">
        <div>
          <h3>ลงประกาศงานของคุณ</h3>
          <p>ยังไม่มีประกาศงานของคุณ เพิ่มประกาศงานของคุณเพื่อค้นหาพนักงานใหม่</p>
          <a href="{{$shopUrl}}job_add" class="button">ลงประกาศงาน</a>
        </div>
      </div>

      @endif

    </div>

  </div>

@stop